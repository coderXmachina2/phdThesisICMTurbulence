...
\end{figure}
\clearpage
\section{A short section on Turbulence theory}\label{Chap:scientificLandscapeTurbulenceTheory}
...
In the inertial range, where energy transfer is local in scale and the energy flux $\epsilon \sim u_l^2 / t_\mathrm{eddy}(l)$ is constant across scales, this gives the Kolmogorov velocity scaling $u_l \propto l^{1/3}$. The corresponding three-dimensional power spectrum of kinetic energy takes the form:
\begin{equation}
	E(k) \propto C_{p} \dot{\epsilon}^{2/3} k^{-5/3},
	\label{eq:kolmogorov}
\end{equation}
\\
where $ C_{p}$ is the universal non-dimensional Kolmogorov constant and $\dot{\epsilon}$ is the turbulent energy dissipation rate per unit mass. This is celebrated but this is also angle averaged also known as the omnidirectional spectrum.
