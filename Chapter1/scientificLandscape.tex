...
Structurally the DM component dictates the overall potential well, setting the stage for all of the baryonic interactions \textcolor{red}{[ref]}. At the base of this immense potential well sits the largest galaxy known as the brightest cluster galaxy (BCG), that accounts for $0.5$--$2\%$ of the cluster baryons. The BCG commonly hosts an active galactic nucleus (AGN), a powerful source of energetic feedback attributed to super massive black hole (SMBH) activity as the origin \textcolor{red}{[ref]}. Understanding AGN physics is of broad interest in modern astrophysics, spanning diverse research areas from exotic accretion processes to high powered jet emission \textcolor{red}{[ref]}, \textcolor{red}{[ref]}, and while not the primary focus of this work, AGN feedback has major implications towards the stirring of gas motions within the ionised ICM.
...
The observational leverage we have on all three of the aforementioned processes is mediated through how the ICM manifests across the electromagnetic spectrum. The next section details the primary observational windows through which the ICM is studied in order to derive constraints on the cluster mass, and sets the groundwork for the key physical quantities and input assumptions that underpin the work done in later chapters.

\newpage
\section{Observables}\label{Chap:scientificLandscapeObservables}

Optical light as the oldest astronomical window was used to initially identify and catalog the first galaxy groups and clusters \textcolor{red}{[ref]}. It remains a viable avenue to evaluate the matter content of galaxy clusters via gravitational lensing \textcolor{red}{[ref]} and velocity dispersion of member galaxies  \textcolor{red}{[ref]}. However the windows of most relevance towards the mapping of the internal thermodynamic states of the ICM are namely the X-ray and millimeter-wave radio respectively. 

\subsection{X-ray emission}\label{Chap:scientificLandscapeObservablesXray}

The X-ray window played a vital role in the initial discovery and characterisation of the ICM. To date the ICM of galaxy clusters remain among the brightest X-ray sources on the sky  \textcolor{red}{[ref]}. The emission is predominantly produced via thermal bremsstrahlung (free-free emission), in which electrons are deflected by ions and radiate photons in the process. The X-ray surface brightness $S_\mathrm{X}$ of the ICM electrons is proportional to the line-of-sight (LOS) emission integral \textcolor{red}{[ref]},
...
\end{equation}
\\
where $n_e$ and $n_p$ are the electron and proton number densities respectively, $\Lambda(T)$ is the plasma cooling function, and the integral is taken along the line of sight $z$. Since the emissivity scales as $n_e^2$, X-ray observations generally map gas density and are more sensitive to the cluster core, making them more suited for detailed high resolution studies of the inner regions of the ICM. However, this sensitivity can be a double-edged sword as the $n_e^2$ dependence means that $S_\mathrm{X}$ can be biased by an unknown gas clumping factor $C_\rho$ arising from inhomogeneous gas distributions in the core \textcolor{red}{[ref]}.
\pbreak
...
\end{equation}
\\
where $M(<r)$ is the total mass enclosed within radius $r$, $\phi$ is the gravitational potential, $\rho_g$ is the gas density, and $P$ is Pressure. This local hydrostatic balance contributes to the cluster’s broader virialized state and provides a key foundational framework through which X-ray observations can be used to infer cluster mass. There has been overall good consensus between traditional X-ray methods using the HE assumption in finding the true cluster mass with maximum root mean square (RMS) deviation ranging from 5 \% in relaxed cores to 20 \% in the outskirts\textcolor{red}{[ref]} \textcolor{red}{[ref]}. \textcolor{blue}{However, there are some limitations of the X-ray method that can be complemented by other wavelengths.}

\newpage
Another important concept that emerged from the X ray regime is the analytical $\beta$-model. By applying an approximation for a self-gravitating collisionless system (originally developed for globular clusters by \textcolor{red}{[ref]}) to a radial model of galaxy and gas distribution in equilibrium within the same potential \textcolor{red}{[ref]} derived the analytic isothermal radial $\beta$ model of the ICM electron number density:
...
Notwithstanding the overview provided thus far, the ICM is in reality a complex and dynamically evolving system. The true gas conditions are in actuality far removed from the idealised equilibrium state assumed in the foundational framework of the previous sections. A truer conceptual picture — one favoured in the present literature — models the ICM as a stratified, multiphase fluid gravitationally confined within the cluster potential well \textcolor{red}{[ref]}\textcolor{red}{[ref]}. We now briefly interrogate each of these concepts in detail.
...
The cumulative effect of this multi-scale energy injection is a turbulent velocity field and fluctuations that permeates the ICM from the virial radius inward. In the next section we consider discus some fundamental physical aspects of the turbulence phenomena.  
\clearpage
\begin{figure}[p]
	\vspace*{\fill}
	\centering
...
The total mass of a galaxy cluster is its most fundamental observable quantity. Together with redshift, it links observational data to theoretical predictions, enabling the cluster population to serve as a probe of both structure formation and cosmological parameters \textcolor{red}{Voit2005, Kravtsov2012, Allen2011}. Yet mass is not directly observable; it must be inferred from the thermodynamic and kinematic properties of baryonic tracers under a set of physical assumptions. The accuracy of those assumptions --- and the systematic biases they introduce --- therefore lies at the heart of cluster cosmology.

In this section we describe the principal theoretical frameworks for cluster mass reconstruction, their observational implementations, and the nature of the biases that arise when the underlying assumptions are violated.

\subsection{The Generalised Mass Equation}
\label{sec:generalised_mass}

The most physically complete description of the cluster mass profile is obtained by integrating the full Euler equation for an inviscid collisional gas over a spherical shell. Inclusive of all of the dynamical terms, the total mass enclosed within radius $r$ can be written as \textcolor{red}{Lau2013}

\begin{equation}
	M(<r) \;=\; M_{\mathrm{therm}} + M_{\mathrm{rand}} + M_{\mathrm{rot}}
	+ M_{\mathrm{cross}} + M_{\mathrm{stream}} + M_{\mathrm{accel}},
	\label{eq:generalised_mass}
\end{equation}
\\
\noindent where each term represents a distinct contribution to the pressure support balancing gravity. The first term, $M_{\mathrm{therm}}$, is the hydrostatic mass arising from isotropic thermal pressure gradients. The second and third terms, $M_{\mathrm{rand}}$ and $M_{\mathrm{rot}}$, account for the additional pressure support provided by random (turbulent) and coherent rotational gas motions, respectively. The remaining terms --- $M_{\mathrm{cross}}$, $M_{\mathrm{stream}}$, and $M_{\mathrm{accel}}$ --- capture contributions from cross-stream motions, bulk inflow or outflow (streaming), and local gas acceleration, all of which become relevant during dynamically disturbed phases such as active mergers or rapid mass accretion.

Equation~\ref{eq:generalised_mass} is the definitive tool for numerical simulation. In a hydrodynamical simulation, all six components can be evaluated directly from three-dimensional velocity and pressure fields, providing complete knowledge of the mass budget at any radius and point of the cluster's assembly history. This makes it invaluable for quantifying the mass bias introduced by specific physical processes: sloshing cold fronts, AGN-driven cavities, merging substructure, and large-scale accretion shocks all leave distinct signatures across these terms. Simulations consistently find that the random-motion term $M_{\mathrm{rand}}$ dominates the non-thermal correction, contributing at the level of $10$--$30\%$ of $M_\mathrm{true}$ at $R_{500}$ and growing toward the cluster outskirts, while the acceleration term $M_{\mathrm{accel}}$ becomes non-negligible only in merging systems or beyond the virial radius \textcolor{red}{Rasia2006, Nagai2007, Lau2009, Nelson2014a, Biffi2016}.

Despite its completeness, Equation~\ref{eq:generalised_mass} is inaccessible to direct observation. The three-dimensional velocity field of the ICM cannot be resolved from a single line-of-sight vantage point, and even with next-generation X-ray spectroscopy \textcolor{red}{[ref]}, spatially resolved velocity measurements will remain limited to the inner cluster regions.

\subsection{The Hydrostatic Mass Equation}
\label{sec:he_mass}

After several sound-crossing times ($\sim 10^9$ yr), a relaxed cluster is expected to approach hydrostatic equilibrium (HE), in which the inward gravitational force is balanced by the outward thermal pressure gradient alone. Setting all non-thermal terms in Equation~\ref{eq:generalised_mass} to zero and adopting spherical symmetry, the Euler equation reduces to the HE equation,

\begin{equation}
	\frac{1}{\rho_{\mathrm{gas}}} \frac{dP_{\mathrm{th}}}{dr}
	= -\frac{G M_\mathrm{HE}(<r)}{r^2},
	\label{eq:he}
\end{equation}
\\
\noindent from which the purely hydrostatic mass profile follows as
...
\noindent Here $k_B$ is the Boltzmann constant, $T(r)$ the gas temperature, $\mu \approx 0.6$ the mean molecular weight of an ionised plasma, $m_p$ the proton mass, and $G$ the gravitational constant. The observationally accessible quantities --- the gas density profile $\rho_{\mathrm{gas}}(r)$, obtained from the deprojection of X-ray surface brightness, and the temperature profile $T(r)$, obtained from spectral fitting --- enter directly through the logarithmic gradients.

Equation~\ref{eq:he_mass} is the workhorse of X-ray cluster cosmology. Combined with the availability of spatially resolved spectroscopy from \textit{XMM-Newton} and \textit{Chandra}, it has been used to derive mass profiles for hundreds of clusters across a wide range of mass and redshift \textcolor{red}{Pointecouteau2005, Vikhlinin2006, Ettori2013}. Its principal strength is practical: it requires only two radial profiles, both measurable with current instrumentation, and returns a continuous mass profile $M(<r)$ without requiring any assumption about the form of the gravitational potential. Its principal weakness is equally clear: by construction it accounts only for thermal pressure support, and any residual non-thermal pressure contribution causes $M_\mathrm{HE}$ to systematically underestimate the true mass.

\subsection{The Non-Thermal Corrected Mass Equation}
\label{sec:nt_mass}

A more complete observational estimate is obtained by retaining the dominant non-thermal contribution, namely the isotropic random gas motions (turbulence). Defining the non-thermal pressure fraction as
...
\noindent the total pressure is $P_{\mathrm{tot}} = P_{\mathrm{th}}/(1 - f_{\mathrm{nt}})$. Substituting into the equation of motion and again assuming spherical symmetry, the mass profile corrected for non-thermal pressure support is
...
\begin{equation}
	...
	\frac{d\ln \rho_{\mathrm{gas}}(r)}{d\ln r}
	+ \frac{d\ln T(r)}{d\ln r}
	+ \frac{d\ln \bigl(1 - f_{\mathrm{nt}}(r)\bigr)^{-1}}{d\ln r}
	\right]
	\times \bigl(1 - f_{\mathrm{nt}}(r)\bigr)^{-1}.
	\label{eq:nt_mass}
\end{equation}

\noindent The third logarithmic gradient term and the prefactor $(1-f_\mathrm{nt})^{-1}$ together encode the correction relative to the pure HE estimate: the gradient term accounts for the \emph{radial variation} of the non-thermal support, which introduces an additional effective pressure contribution, while the prefactor accounts for the \emph{amplitude} of the non-thermal support at radius $r$. In the limit $f_{\mathrm{nt}} \rightarrow 0$, Equation~\ref{eq:nt_mass} reduces identically to Equation~\ref{eq:he_mass}, as required. For a cluster with $f_{\mathrm{nt}} = 0.2$ at $R_{500}$ and a mild radial gradient, the corrected mass exceeds the purely hydrostatic value by $\sim 25\%$, broadly consistent with the range predicted by numerical simulations \textcolor{red}{Nelson2014a, Biffi2016} and inferred from X-ray and SZ comparisons \textcolor{red}{Eckert2019}.
...
The systematic offset between the hydrostatic mass estimate and the true mass is quantified by the \emph{hydrostatic mass bias},

\begin{equation}
	b \;\equiv\; 1 - \frac{M_{\mathrm{HE}}}{M_{\mathrm{true}}},
	\label{eq:mass_bias}
\end{equation}

\noindent so that $b > 0$ implies an underestimate of the true mass. In the limit where non-thermal pressure is the sole source of bias, and assuming $f_\mathrm{nt}$ is slowly varying with radius, $b \approx f_{\mathrm{nt}}$. More generally, $b$ also absorbs contributions from gas temperature inhomogeneities, gas clumping, departures from spherical symmetry, and absolute X-ray temperature calibration uncertainties \textcolor{red}{Rasia2014, Pratt2019}.

Numerical simulations are unanimous in predicting a non-zero and radially increasing hydrostatic bias. Studies consistently find $b \lesssim 5$--$10\%$ in cluster cores and $b \sim 20$--$30\%$ near $R_{500}$, with the amplitude depending strongly on dynamical state and mass accretion rate \textcolor{red}{Nagai2007, Nelson2014a, Biffi2016}. Observationally, the bias is most directly constrained by comparing hydrostatic masses to weak gravitational lensing masses, which are free from assumptions about the dynamical state of the gas. Such comparisons yield values in the range $1 - b \approx 0.74$--$0.95$, with systematic differences at the $\sim 10\%$ level between different lensing programmes \textcolor{red}{Mahdavi2013, Hoekstra2015, Smith2016}.

The mass bias carries direct consequences for cluster cosmology. Because the cluster mass function is exponentially sensitive to mass near the high-mass end, a systematic underestimate of cluster masses shifts the inferred values of $\sigma_8$ and $\Omega_m$ \textcolor{red}{Vikhlinin2009, PlanckXX2014}. The well-known tension between the number counts of Planck SZ-selected clusters and the primary CMB constraints on $\sigma_8$ can be partially or fully resolved by invoking a hydrostatic bias of order $b \sim 0.4$--$0.6$ \textcolor{red}{PlanckXXIV2016}, a value substantially larger than simulation predictions. Whether this discrepancy reflects unmodelled non-thermal physics, an underestimate of systematics in either probe, or new physics (e.g. a non-zero summed neutrino mass) remains an open question and is one of the primary motivations for improved cluster mass calibration.

\subsection{Mass Proxies and Scaling Relations}
\label{sec:mass_proxies}

While continuous mass equations of the form of Equations~\ref{eq:he_mass} and \ref{eq:nt_mass} provide physically rigorous mass profiles for individual clusters, they demand spatially resolved spectroscopic data of sufficient quality that their application is necessarily limited to a few hundred well-observed objects at most. Cosmological analyses of large cluster samples instead rely on empirically calibrated \emph{mass proxies}: global observables that correlate tightly with mass and can be measured from lower-quality or unresolved data.
