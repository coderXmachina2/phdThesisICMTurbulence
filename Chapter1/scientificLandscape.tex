...
Notwithstanding the overview provided thus far, the ICM is in reality a complex and dynamically evolving system. The true gas conditions are in actuality far removed from the idealised equilibrium state assumed in the foundational framework described in the previous sections. A truer conceptual picture — one favoured in the present literature — models the ICM as a stratified, multiphase fluid gravitationally confined within the cluster potential well \textcolor{red}{[ref]}\textcolor{red}{[ref]}. We now briefly interrogate each of these concepts in turn.
...
\section{Mass estimation and mass calibration and mass bias}

All about that mass no treble. Some more equations about some mass enclosed within a certain radius R. So we talked alot of game on ICM physics and non thermal pressure as deviations for HE. Now we talk about their implications on mass measurements. This is the generalized (complete) mass equation derived from the full Eurler description of fluid:

\begin{equation}
M(<R) = M_{\mathrm{therm}} + M_{\mathrm{rand}} + M_{\mathrm{rot}} + M_{\mathrm{cross}} + M_{\mathrm{stream}} + M_{\mathrm{accel}}
\end{equation}
\\
This is the integrated mass equation. It is actually a subset of the generalized math equation.
\begin{equation}
	M(<R) = \mathrm{Some ol bullshit}
\end{equation}
\\
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
...
\end{itemize}led as follows: Chapter 2 looks at the present pipeline framing the current assumptions, Chapter 3 looks at some methods we have done to interrogate those assumptions, Chapter 4 introduces a new method to circumvent some of those biases. Chapter 5 presents results and discussions.